...
def dht(my_board, callback=None):
    """
     ...
     :param my_board: a pymata_rh instance
     :param callback: callback funtion
     """

    # set the pin mode - for pin 6 differential is set explicitly
    my_board.set_pin_mode_dht(14, sensor_type=22, differential=.01, callback=callback)
    my_board.set_pin_mode_dht(15, sensor_type=11, callback=callback)
    ...
    while True:
        try:
            ...
            # poll the first DHT
            value = board.dht_read(14)

            # format the time string and then print the data
            tlist = time.localtime(value[2])
            ftime = f'{tlist.tm_year}-{tlist.tm_mon:02}-{tlist.tm_mday:02} ' \
                    f'{tlist.tm_hour:02}:{tlist.tm_min:0}:{tlist.tm_sec:02}'
            print(f'poll pin 14: humidity={value[0]} temp={value[1]} '
                  f'time of last report: {ftime}')

            # poll the second DHT and print the values
            value = board.dht_read(15)
            tlist = time.localtime(value[2])
            ftime = f'{tlist.tm_year}-{tlist.tm_mon:02}-{tlist.tm_mday:02} ' \
                    f'{tlist.tm_hour:02}:{tlist.tm_min:0}:{tlist.tm_sec:02}'
            print(f'poll pin 15: humidity={value[0]} temp={value[1]} '
                  f'time of last report: {ftime}')
            if not changed:
                # explicitly change the differential values
                my_board.set_pin_mode_dht(14, sensor_type=22, differential=20.0, callback=callback)
                my_board.set_pin_mode_dht(15, sensor_type=11, differential=2.0, callback=callback)
                changed = True
