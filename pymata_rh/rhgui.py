...
class RHGui:
    ...
    def killall(self):
        ...
        if self.proc_gui:
            try:
                if sys.platform.startswith('win32'):
                    subprocess.run(['taskkill', '/F', '/t', '/pid', str(self.proc_gui.pid)],
                                   creationflags=subprocess.CREATE_NEW_PROCESS_GROUP |
                                                 subprocess.CREATE_NO_WINDOW
                                   )
                else:
...
def rhgui():
    ...
    RHGui(com_port=com_port, arduino_instance_id=args.arduino_instance_id)
